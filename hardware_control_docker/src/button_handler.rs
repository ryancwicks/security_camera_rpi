...
pub async fn _button_task (led_rx: mpsc::Sender<ButtonCommand>) {
                ...
                match event {
                    Ok(_) =>{
                        ...
                        if button_read.get_value().unwrap_or( 0 ) == 1 { //if still pressed after 10 ms then properly debounced.
                            led_rx.send(ButtonCommand::_ButtonPressed).await.unwrap_or_else(|_| log::error!("Failed to send button pressed command to manager task."));
                        }
                    },
